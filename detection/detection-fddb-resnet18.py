...
def main():
    fp = open('/disk2/data/FACE/fddb/originalPics/fddb_test_list.txt','r')
    file = open("fddb_result-resnet18-zyf.txt",'w')
    ...
    sym = resnet_18(num_class=2)

    for line in all_lines:
        print count
        count = count + 1
        print line
        color = cv2.imread(line[:-1])
        print color.shape
        ...
        for i in range(dets.shape[0]):
            file.write('{} {} {} {} {}\n'.format(dets[i,0],dets[i,1],dets[i,2],dets[i,3],dets[i,4]))
        file.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prefix', type=str, default='faster-resnet-18', help='the prefix of the pre-trained model')
